...
@interface QPDaterView ()<UIPickerViewDelegate,UIPickerViewDataSource>{
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component __TVOS_PROHIBITED{
    if (_dateViewType == QPDateViewTypeDate) {
        NSUInteger mid = 0;
        ...
        self.dateString = [NSString stringWithFormat:@"%d-%02d-%02d",_year,_month,_day];
    }else if (_dateViewType == QPDateViewTypeTime){
        NSUInteger mid = 0;
        NSUInteger rowIndex = 0;
        if (component==0) {
            mid = [self midNum:24];
...
- (void)setDateViewType:(QPDateViewType)dateViewType{
